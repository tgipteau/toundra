...
ELMT_SIZE = (
    0.6 * ISO_SCALE
)  # taille des arbres en ratio (peut être > 1) par rapport à iso_scale
...
minree = float(params["minree"])
maxree = float(params["maxree"])
...
def run_simulation(save_folder=SIM_NAME):
    ...
    diagsup = np.ones(J - 1)
    D = (
        np.diag(diag * (1 + 2 * (kx + ky)), 0)
        + np.diag(diagsup * (-ky), 1)
        + np.diag(diagsup * (-ky), -1)
    )
    A = block_diag(D)
    for i in range(J - 1):
    ...
    for t in tqdm(range(T)):
        ...
        if t % freq == 0:
            if np.random.binomial(1, p) == 1:
                ...
                var_ee = lambda i, j, var: (
                    var[i * J + j] * (1 - intensity)
                    if ((i - xee) ** 2 + (j - yee) ** 2) < ree**2
                    else var[i * J + j]
                )
...
def get_dfs(from_folder="Simulations/default"):
    df = pd.read_csv(
        os.path.join(from_folder, "data.csv"),
        names=["t", "x", "y", "u", "w"],
        header=0,
        sep=" ",
        index_col=False,
    ...
    df_feux = pd.read_csv(
        os.path.join(from_folder, "fires.dat"),
        names=["t", "x", "y", "r", "I"],
        header=None,
        sep=" ",
        index_col=False,
...
slider_rect = pygame.Rect(
    (10, SCREEN_HEIGHT - ui_height), (SCREEN_WIDTH - 20, ui_height)
)
...
slider = pygame_gui.elements.UIHorizontalSlider(
    ...
    value_range=(
        0,
        0,
    ),  # value range adaptée à MAX_T dans main seulement (MAX_T pas encore valué)
    manager=manager,
)
...
def build_from_solution(saved_simulation_folder="Simulations/default"):
    ...
    unique_xy_list = list(
        df[["x", "y"]].drop_duplicates().itertuples(index=False, name=None)
    )
...
class Tile:
    ...
    def __init__(self, topleft_node):
        ...
        self.iso_x, self.iso_y = projeter_en_isometrique(
            self.topleft_node.x, self.topleft_node.y
        )
        ...
        self.corners.append(
            (self.topleft_node.x + TILE_SIZE, self.topleft_node.y + TILE_SIZE)
        )
        self.corners.append((self.topleft_node.x, self.topleft_node.y + TILE_SIZE))

        self.center = (
            self.topleft_node.x + TILE_SIZE // 2,
            self.topleft_node.y + TILE_SIZE // 2,
        )
        ...
        self.screen_nb_trees = [
            int(
                (true_nb_trees_t - TREES_T_MIN)
                * MAX_ELMTS_PER_TILE
    ...
    def generate_trees(self):
        ...
        while len(generated_points) < MAX_ELMTS_PER_TILE:
            x, y = (
                random.uniform(self.topleft_node.x, self.topleft_node.x + TILE_SIZE),
    ...
    def draw_tile(self, screen):
        ...
        points_iso = [
            projeter_en_isometrique(corner[0], corner[1]) for corner in self.corners
        ]
    ...
    for fire in fires:
        ...
        for tile in tiles:
            ...
            if dif_x**2 + dif_y**2 < fire.r**2:
                ...
                nb_to_burn = int(
                    len(eligible_trees) * fire.I
                )  # nb d'arbres qui brûlent parmi les eligibles
...
if __name__ == "__main__":
    ...
    lines = read_params_from(
        os.path.join(SIMULATION_SAVE_FOLDER, "params.txt")
    )  # à adapter
